...
def excel_entry(specs, file_name, sheet, format):
    ...
    for i in specs:
        ...
        for r in range(2000,sheet.max_row+2):
            if check_pass == 1:
                break
            if sheet.cell(row=r,column=end_column).value == None:
                check_pass = 1
                max_row = r
        if y==1:
                sheet.cell(row=max_row,column=y).value = specs[i]
                sheet.cell(row=max_row,column=y).alignment = Alignment(horizontal='center')
                y += 1
                continue
        else:
            if not y in nonfloat_list:
                try:
                    sheet.cell(row=max_row,column=y).value = float(specs[i])
                    sheet.cell(row=max_row,column=y).alignment = Alignment(horizontal='center')
                except ValueError:
